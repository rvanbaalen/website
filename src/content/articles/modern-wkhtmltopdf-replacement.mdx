...
One thing to plan for: your documents render on a modern engine now, so they won't be pixel-identical to the old output. Chromium renders your CSS as written — no more silent down-scaling of slightly-too-wide content — so text metrics and page breaks can shift, and a layout that leaned on that old behavior may want a small adjustment. (Those `--zoom 1.5` flags in legacy templates were compensating for exactly this.)

The migration is straightforward: install the binary, render your documents through the new and old versions, compare side by side, and adjust the few spots that moved — typically a margin or a zoom. Then ship it. It's an afternoon of work, not a rewrite.
