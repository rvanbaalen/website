...
One thing worth knowing going in: your documents will render like it's 2026, not 2011, which means they won't be pixel-identical to the old output. That's the whole point — but it does mean a slightly wider character here, a different line-wrap there, and on a long report those small differences can shift where a page breaks. wkhtmltopdf also used to silently scale slightly-too-wide content down to fit; a modern engine renders your CSS as written instead, so a layout that quietly leaned on that trick might need a real fix. (Those `--zoom 1.5` flags in old templates? That's what they were papering over.)

So the migration is short and boring: install the new binary, run your real documents through it and the old one, put the PDFs side by side, and touch up the handful of spots that moved — usually a margin or a zoom, often nothing at all. Then approve them and move on. That's the whole job, and next to rewriting a production PDF layer it's an afternoon.
