...
There's one catch I can't engineer away, so I'll just be upfront about it: the same HTML renders a little differently on a new engine. Chromium measures text differently than 2011 WebKit did — a slightly different character width here, a different line-wrap there — and on a long document those small differences add up and can shift where a page breaks. wkhtmltopdf also had a "smart shrinking" trick that silently scaled slightly-too-wide content down to fit; Chromium doesn't, so a layout that quietly leaned on that can now overflow instead. (If you've ever seen a `--zoom 1.5` in an old template, that's what it was compensating for.)

None of that is fixable, because it isn't a bug — it's just what happens when you leave a fourteen-year-old engine, and any modern replacement would do the same. So the migration is concrete and boring: install the new binary, run your real documents through both it and the old one, put the PDFs side by side, and fix the few spots that moved — usually a margin or a zoom. Then approve them. That's the whole job. Next to rewriting a production PDF layer, it's an afternoon.

The binary in my basement is called wkhtmltopdf again. The code that calls it never noticed a thing. The designer's container query renders. And the boring, load-bearing, absolutely-critical job of turning HTML into a payslip is getting done again — same tool, same command, newer brain. Which is all I ever wanted from it.
