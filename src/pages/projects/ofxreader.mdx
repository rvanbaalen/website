...
metaDescription: "Read OFX 2.x (XML) bank and credit-card statements and get clean, deterministic JSON. Ships a CLI and an MCP server so LLM agents can query statements directly."
...
Install from npm — it's a public package, no authentication needed:
...
It's also published to GitHub Packages. Unlike npm, GitHub Packages requires an authenticated token even for public packages, so point the `@rvanbaalen` scope at the GitHub registry first:
